...
import com.basdat.controller.customer_controller.LoginCustomerController;
...
public class AkunCustomerController implements Initializable {
    ...
    private void updateInfo() {
        String email, NIK, nama, jenisKelamin, jalan, kecamatan, kota, username;

        email = LoginCustomerController.getEmail();
        NIK = LoginCustomerController.getNIK();
        nama = LoginCustomerController.getNama();
        jenisKelamin = LoginCustomerController.getJenisKelamin();
        jalan = LoginCustomerController.getJalan();
        kecamatan = LoginCustomerController.getKecamatan();
        kota = LoginCustomerController.getKota();
        username = LoginCustomerController.getUser();
    ...
    private void newPassBtnAction() {
        ...
        if(!(newPass.isBlank() && newPass1.isBlank()) && (newPass.equals(newPass1)) && !(newPass.equals(LoginCustomerController.getPass()))) {
            try {
                setNewPass(newPass);
        ...
        else if (newPass.equals(LoginCustomerController.getPass())) {
    ...
    private void setNewPass (String newPass) {
        String ID_Pengguna = LoginCustomerController.getID_Pengguna();
